...
$app->group('', function ($group): void {
    ...
    $group->put('/companies', CompanyController::class . ':update');
    $group->get('/suppliers', SupplierController::class . ':index');
    $group->post('/suppliers', SupplierController::class . ':store');
    $group->get('/suppliers/{id:[0-9]+}', SupplierController::class . ':show');
    $group->put('/suppliers/{id:[0-9]+}', SupplierController::class . ':update');
    $group->patch('/suppliers/{id:[0-9]+}/status', SupplierController::class . ':status');
    $group->get('/cashier', CashierController::class . ':index');
    $group->post('/cashier/open', CashierController::class . ':open');
    $group->post('/cashier/{id:[0-9]+}/movements', CashierController::class . ':movement');
    $group->post('/cashier/{id:[0-9]+}/close', CashierController::class . ':close');
    $group->get('/settings', SettingsController::class . ':index');
    $group->put('/settings/preferences', SettingsController::class . ':savePreferences');
    $group->put('/settings/finance', SettingsController::class . ':saveFinance');
